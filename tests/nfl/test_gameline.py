...
def test_bias_default_zero_is_noop():
    # default bias 0.0 leaves the model-only estimate unchanged
    row = build_gameline(model_margin=3.0, model_total=45.0,
                         market={"spread_line": None, "total_line": None}, week=1, cfg=CFG)
    assert math.isclose(row["pred_margin"], 3.0)
    assert math.isclose(row["pred_total"], 45.0)


def test_bias_subtracted_from_model_estimate():
    # model runs high: bias_margin +2, bias_total +4 -> subtracted (model-only)
    cfg = GameLineConfig(sigma_margin=13.2, sigma_total=10.0, offset=75, total_max=120,
                         bias_margin=2.0, bias_total=4.0)
    row = build_gameline(model_margin=3.0, model_total=45.0,
                         market={"spread_line": None, "total_line": None}, week=1, cfg=cfg)
    assert math.isclose(row["pred_margin"], 1.0)   # 3 - 2
    assert math.isclose(row["pred_total"], 41.0)   # 45 - 4
